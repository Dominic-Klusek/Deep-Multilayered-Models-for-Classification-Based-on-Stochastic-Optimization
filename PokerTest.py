...
def filenameappend(activationList, batchSize, fileName = "PokerResults", extension=".txt"):
    '''
...
#batchSizes = [128, 128, 128, 128, 128, 128, 128, 128, 128, 128]
#batchSizes = [1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024]
batchSizes = [2693,2693,2693,2693,2693,2693,2693,2693]
...
data = np.genfromtxt('Poker Dataset/poker.t', delimiter=' ', max_rows=250000)
X = np.array(data[:,1:])
Y = np.array(data[:,0])
print(len(X))

#create an instance of an excel workbook and set the row number of the data to begin at
workbook = xlsxwriter.Workbook(str(numLayers[0] - 1) + ' ' + 'PokerTest'+str(batchSizes[0])+'Selu.xlsx')
row = 2
...
for actList, index, batch, optimizer in zip(activationList, range(0, len(activationList)), batchSizes, optimizerList):
    ...
    # define 8-fold cross validation test harness
    ...
    plt.ylabel("Accuracy %")
    plt.xlabel("Epoch")
    plt.title("Poker Accuracy over Epoch Validation")

    lossPlotVal = plt.figure(2)
    ...
    plt.ylabel("Loss")
    plt.xlabel("Epoch")
    plt.title("Poker Loss over Epoch Validation")
...
file = open(str(batchSizes[0]) + ' ' + str(numLayers[0] - 1) + ' Hidden Layers PokerAccValFig Selu.pickle','wb')
pickle.dump(accPlotVal, file)
file.close()

file = open(str(batchSizes[0]) + ' ' + str(numLayers[0] - 1) + ' Hidden Layers PokerLossValFig Selu.pickle','wb')
